...
const Display = () => {
    ...
    // Split the sequence string into an array of numbers
    var datapoint = [];
    // var sequenceArray = sequenceString.split(" ").map(Number);
    for (let i = 1; i <= 795; i++) {
    ...
    return (
        <Chart options={{
            ...
            series={[{
                name: 'ecg val',
                data: datapoint && datapoint.map((item) => (
                    item.y
                )),
